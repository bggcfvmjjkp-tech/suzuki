...
function HomeContent() {
  ...
  return <main>
    ...
    <section className="bg-white pb-10 md:pb-14"><div className="mx-auto max-w-[1400px] px-5"><SectionTitle>Các xe du lịch Suzuki Vinh Nghệ An</SectionTitle>{isCarsLoading ? <div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-[#004a9f] border-t-transparent" /> : <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 lg:grid-cols-4">{currentCars.map((car) => <CarCard key={car.slug || car.name} car={car} />)}</div>}</div></section>
    <section className="bg-[#eef2f6] py-10 md:py-14"><div className="mx-auto max-w-[1400px] px-5"><SectionTitle>Các xe thương mại Suzuki Vinh Nghệ An</SectionTitle>{isCarsLoading ? <div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-[#004a9f] border-t-transparent" /> : <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 lg:grid-cols-4">{serviceCars.map((car) => <CarCard key={car.slug || car.name} car={car} />)}</div>}</div></section>
